...
GOVERSION=${1:-1.10}
REPO=ucd
DOCKER_IMAGE_TAG=UtopiaCoinOrg-golang-builder-$GOVERSION
...
if [ -f ~/.cache/$DOCKER_IMAGE_TAG.tar ]; then
	...
	docker pull UtopiaCoinOrg/$DOCKER_IMAGE_TAG
	if [ $? != 0 ]; then
		echo 'docker pull failed'
		exit 1
	fi
	docker save -o ~/.cache/$DOCKER_IMAGE_TAG.tar UtopiaCoinOrg/$DOCKER_IMAGE_TAG
	if [ $? != 0 ]; then
...
docker run --rm -it -v $(pwd):/src UtopiaCoinOrg/$DOCKER_IMAGE_TAG /bin/bash -c "\
  rsync -ra --filter=':- .gitignore'  \
  /src/ /go/src/github.com/UtopiaCoinOrg/$REPO/ && \
  cd github.com/UtopiaCoinOrg/$REPO/ && \
